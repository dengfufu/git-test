 ...
 **/
@Component(AbstractIncomeStrategy.INCOME_SALE_LOAN_RETURN)
public class IncomeSaleLoanReturnStrategy extends AbstractIncomeStrategy {
    @Autowired
    ...
    protected void addIncome(IncomeWareCommonDto incomeMainCommonDto, UserInfo userInfo, ReqParam reqParam) {
            ...
            for (int i = 0; i < incomeDetailCommonSaveList.size(); i++) {
                ...
                if (CollectionUtil.isNotEmpty(incomeDetailCommonSaveDto.getBookIdList())) {
                    incomeDetailCommonSaveDto.getBookIdList().forEach(bookId -> {
                        ...
                        bookSaleBorrow.setReverseDetailId(incomeWareCommon.getId());
                        bookSaleBorrow.setReversed(CompletedEnum.YES.getCode());
                        bookSaleBorrow.setReverseBy(incomeWareCommon.getCreateBy());
                        bookSaleBorrow.setReverseTime(DateUtil.date().toTimestamp());
        ...
        if (CollectionUtil.isNotEmpty(bookSaleBorrowList)) {
