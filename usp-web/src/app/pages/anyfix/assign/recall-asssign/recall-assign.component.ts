...
@Component({
  selector: 'app-recall-assign',
  templateUrl: './recall-assign.component.html',
  styleUrls: ['./recall-assign.component.less'],
})
export class RecallAssignComponent implements OnInit {
  ...
  getReasonList(){
    this.loading = true;
    this.httpClient
      .get('/api/anyfix/service-reason/list/2')
      .pipe(
        finalize(() => {
          this.loading = false;
  ...
  submitForm(): void {
    ...
    this.loading = true;
    this.httpClient
      .post('/api/anyfix/work-transfer/service/assign/recall', this.getPageParam())
      .pipe(
        finalize(() => {
          this.loading = false;
          this.cdf.markForCheck();
