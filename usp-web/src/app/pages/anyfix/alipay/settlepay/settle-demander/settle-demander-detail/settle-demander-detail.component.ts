...
export class SettleDemanderDetailComponent implements OnInit {
  ...
  querySettleDemanderDetail(reset?: boolean) {
    ...
    this.httpClient.post('/api/anyfix/settle-demander-detail/query', params)
      .pipe(
        finalize(() => {
  ...
  // 是否显示开票按钮
  showInvoice(): boolean {
    return this.settleDemander.serviceCorp === this.corpId
      && this.settleDemander.status === 2
  ...
  // 是否显示付款按钮
  showPay(): boolean {
    return this.settleDemander.demanderCorp === this.corpId
      && this.settleDemander.status === 2
