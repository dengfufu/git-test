...
import {ServiceCheckListFilterComponent} from './filter/service-check-list-filter.component';
...
@Component({
  selector: 'app-work-service-check-list',
  templateUrl: './service-check-list.component.html',
  styleUrls: ['./service-check-list.component.less']
})
export class ServiceCheckListComponent implements OnInit {
  ...
  @ViewChild(ServiceCheckListFilterComponent, {static: false}) serviceCheckListFilterComponent: ServiceCheckListFilterComponent;
  ...
  url = '/api/anyfix/work-request/service-check/query';
  ...
  ngOnInit() {
    ...
    if (params && params._type === 'work-service-check') {
      this.params = params;
  ...
  initFormValue() {
    this.searchForm.patchValue({
      finishCheckStatuses: this.params.finishCheckStatuses,
      finishConfirmStatuses: this.params.finishConfirmStatuses,
      finishCheckUser: this.params.finishCheckUser,
      finishCheckTime: this.params.finishCheckTime,
      finishCheckStartDate: this.params.finishCheckStartDate,
      finishCheckEndDate: this.params.finishCheckEndDate,
  ...
  queryWork() {
    // 工单筛选数据收集
    this.serviceCheckListFilterComponent.submit();
    this.stPro.reset();
  }
  ...
  getParams() {
    ...
    if (params.finishCheckStatuses === undefined) {
      params.finishCheckStatuses = this.workCheckService.TO_CHECK + ',' + this.workCheckService.CHECK_REFUSE;
    }
    if (params.finishConfirmStatuses === undefined) {
      params.finishConfirmStatuses = this.workCheckService.CONFIRM_REFUSE;
    }
    return params;
  }
  ...
  toWorkDetail(workId) {
    ...
    this.params._type = 'work-service-check';
  ...
  checkPassSubmit(idList) {
    ...
    this.loading = true;
    this.httpClient.post('/api/anyfix/work-check/service/batch-check', params)
      .pipe(finalize(() => {
        this.loading = false;
      }))
