...
@Component({
  selector: 'work-fee-confirm-list-filter',
  templateUrl: 'fee-confirm-list-filter.component.html',
  styleUrls: ['fee-confirm-list-filter.component.less']
})
export class FeeConfirmListFilterComponent implements OnInit {
  ...
  submit() {
    ...
    if (params.feeConfirmTime && params.feeConfirmTime.length === 2) {
      params.feeConfirmStartDate = params.feeConfirmTime[0];
      params.feeConfirmEndDate = params.feeConfirmTime[1];
    } else {
      params.feeConfirmStartDate = null;
      params.feeConfirmEndDate = null;
    }
  ...
  matchFeeConfirmUser(userName?: string) {
    this.listUser(userName).then((userList: any[]) => {
      this.feeConfirmUserList = userList;
    });
  }
