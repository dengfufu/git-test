...
@Component({
  selector: 'app-custom-recall',
  templateUrl: 'custom-recall.component.html',
  styleUrls: ['custom-recall.component.less'],
})
export class CustomRecallComponent implements OnInit {
  ...
  getReasonList(){
    this.loading = true;
    this.httpClient
      .get('/api/anyfix/custom-reason/list/1')
      .pipe(
        finalize(() => {
          this.loading = false;
  ...
  submitForm(): void {
    ...
    this.loading = true;
    this.httpClient
      .post('/api/anyfix/work-transfer/custom/recall', this.getPageParam())
      .pipe(
        finalize(() => {
          this.loading = false;
          this.cdf.markForCheck();
