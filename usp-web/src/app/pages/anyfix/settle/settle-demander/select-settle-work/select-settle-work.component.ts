...
@Component({
  ...
  templateUrl: './select-settle-work.component.html',
  styleUrls: ['./select-settle-work.component.less']
})
export class SelectSettleWorkComponent implements OnInit {
  ...
  updateCheckedSet(id: string, checked: boolean, work): void {
    if (checked) {
      this.setOfCheckedId.add(id);
      this.detailList.push(work);
    } else {
      this.setOfCheckedId.delete(id);
      this.detailList = this.detailList.filter(item => item.workId !== id);
    }
  }

  onAllChecked(value: boolean): void {
    this.workFeeList.forEach(item => this.updateCheckedSet(item.workId, value, item));
  ...
  onItemChecked(id: string, checked: boolean, work): void {
    this.updateCheckedSet(id, checked, work);
    this.refreshCheckedStatus();
  }
