...
@Component({
  selector: 'app-ware-model-edit',
  templateUrl: 'ware-model-edit.component.html'
})
export class WareModelEditComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.validateForm.patchValue(this.wareModel);
    // 后端Y N 转换成前端true false
    this.enabledValue = this.wareModel.enabled === 'Y' ? true : false;
    this.initData();
  }
  ...
  submitForm(value) {
    ...
    this.httpClient.post('/api/wms/ware-model/update', value)
      .subscribe(() => {
