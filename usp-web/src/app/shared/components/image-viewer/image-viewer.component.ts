...
import {IvViewerType} from './types/iv-viewer.type';
import {ImgViewerType} from './types/img-viewer.type';
...
export class ImageViewerComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  prevImg(): void {
    if (this.currentImgIndex <= 1) {
      ...
      this.nzMessageService.info('已到第一条。');
      return;
    }
  ...
  nextImg(): void {
    if (this.currentImgIndex >= this.imgTotal) {
      this.nzMessageService.remove();
      this.nzMessageService.info('已到最后一条。');
      return;
    }
    this.isVertical = false;
    this.currentImgIndex++;
  ...
  private getScale(): number {
    ...
    const currentImgWidth = this.element.querySelector('.iv-small-image').clientWidth;
    return imgViewerHeight / currentImgWidth;
